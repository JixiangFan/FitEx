...
class DataVisualization3 extends Component {
    ...
    get(child(dbRef, `profile/${user["uid"]}`))
      .then((snapshot) => {
        if (snapshot.exists()) {
          ...
          goal = snapshot.val().stepGoal / 2000;
          //console.log(goal);
               ...
               if (fitData) {
                 ...
                 if (lastSyncTime - local_morning >= 0) {
                   user_fitbit_distance =
                     fitData[lastSyncTime]["activity"]["summary"]["distances"];
                   //console.log(user_fitbit_distance);
                   const found = user_fitbit_distance.find((obj) => {
